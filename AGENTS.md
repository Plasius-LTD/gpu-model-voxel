...
### Local Notes
This repo is `@plasius/gpu-model-voxel`, the browser-safe TypeScript owner of
PVOX compilation, validation, CPU traversal, review rendering, and disposable
render-cache derivation.
...
- Treat model documents and PVOX bytes as hostile bounded data. Reject malformed
  records before allocation and never execute, fetch, or interpret embedded
  instructions or external references.
- PVOX is the asset of record. Derived surface caches are disposable and must
  never be described or published as native sparse-field rendering evidence.
